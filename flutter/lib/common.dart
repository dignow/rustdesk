...
String getWindowName({WindowType? overrideType}) {
  switch (overrideType ?? stateGlobal.windowId) {
    case WindowType.Main:
      return "RustDesk";
...
Future<List<Rect>> getScreenListNotWayland() async {
  ...
  try {
    for (var display in jsonDecode(displays) as List<dynamic>) {
      // to-do: scale factor ?
      screenRectList.add(Rect.fromLTWH(
          display['x'], display['t'], display['w'], display['h']));
    }
  } catch (e) {
...
tryMoveToScreenAndSetFullscreen(Rect? screenRect) async {
  ...
  final frame =
      Rect.fromLTWH(screenRect.left + 5, screenRect.top + 5, 600, 400);
